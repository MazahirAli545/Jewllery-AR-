...
The demo now doubles as a jewellery try-on studio. The carousel lets you switch between necklaces (anchored to the neck landmark) and forehead accessories such as a maang tikka. Procedural meshes are still available, but you can also reference richer GLTF/GLB assets.

### Importing custom GLTF jewellery

1. Place your `scene.gltf`/`.glb` (and textures) anywhere inside `3dmodel/`.
2. Add a new `<li>` entry inside `#jewelry-list` in `index.html` and set:
   - `data-3d-type="gltf"`
   - `data-3d-src="3dmodel/your-folder/scene.gltf"`
   - `data-accessory-type="necklace"` or `forehead` plus optional offsets/scales.
3. Run `npx http-server -p 3001` (or any static server), reload, and toggle “Try it On”.

Need sample assets fast? `python scripts/build_gltf.py` re-generates the lightweight demo necklace and maang tikka models included here.
...
- [3D frames generated in less than 10 seconds](https://sketchfab.com/3d-models/3d-frames-generated-in-less-than-10-seconds-5cc3b37589ba43148352c850a764b2db) by VReeAI
